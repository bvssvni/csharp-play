...
namespace Play
{
	...
	public class Group : List<int>
	{
		...
		{
			return Predicate (delegate(Li li){return li.Start <= time && li.End > time;}, data);
		}
		...
		public static Group Union(Group a, Group b)
		{
			...
			int k;
			if (a_length == 0) {
				for (k = 0; k < b_length; k++) list.Add (b[k]);
				
				return list;
			}
			if (b_length == 0) {
				for (k = 0; k < a_length; k++) list.Add (a[k]);
